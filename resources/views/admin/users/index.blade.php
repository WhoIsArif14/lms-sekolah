        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
                <form action="{{ route('admin.users.index') }}" method="GET" class="flex gap-2">
                    ...
                    <button type="submit" class="bg-gray-800 text-white px-6 py-2 rounded-xl hover:bg-black transition font-bold text-sm shadow-sm">Cari</button>
                </form>

                <a href="{{ route('admin.users.create') }}" class="bg-indigo-600 text-white px-6 py-2 rounded-xl hover:bg-indigo-700 font-bold shadow-lg shadow-indigo-100 transition">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full table-auto">
                            <thead class="bg-gray-50/50">
                                <tr>
                                    <th class="px-6 py-4 text-left text-[10px] font-black uppercase text-gray-400 tracking-widest">Informasi Pengguna</th>
                                    @if($tab == 'siswa')
                                        <th class="px-6 py-4 text-left text-[10px] font-black uppercase text-gray-400 tracking-widest">Orang Tua</th>
                                    @endif
                                    <th class="px-6 py-4 text-right text-[10px] font-black uppercase text-gray-400 tracking-widest">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100">
                                ...
                                <tr class="hover:bg-gray-50/50 transition">
                                    ...
                                    @if($tab == 'siswa')
                                        <td class="px-6 py-4">
                                            @if($user->parent)
                                                <span class="text-sm text-gray-700 font-medium">{{ $user->parent->name }}</span>
                                            @else
                                    ...
                                    <td class="px-6 py-4 text-right space-x-2">
                                        <a href="{{ route('admin.users.edit', $user) }}" class="inline-flex items-center justify-center w-9 h-9 bg-amber-50 text-amber-600 rounded-lg hover:bg-amber-100 transition shadow-sm">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                        </a>
                                        <form action="{{ route('admin.users.destroy', $user) }}" method="POST" class="inline">
                                            @csrf @method('DELETE')
                                            <button class="inline-flex items-center justify-center w-9 h-9 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition shadow-sm" onclick="return confirm('Hapus user ini?')">
