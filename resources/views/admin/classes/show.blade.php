        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div class="md:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                    ...
                    <form action="{{ route('admin.classes.import', $class->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                        @csrf
                        <div class="flex items-center justify-center w-full">
                            <label class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-xl cursor-pointer bg-gray-50 hover:bg-gray-100 transition">
                                ...
                                <input type="file" name="file" class="hidden" accept=".xlsx,.xls,.csv" required onchange="this.form.submit()" />
                            </label>
                        </div>
                ...
                <div class="bg-indigo-600 p-6 rounded-2xl shadow-lg text-white flex flex-col justify-center">
                    <p class="text-indigo-100 text-sm font-medium">Total Siswa Terdaftar</p>
                    <h4 class="text-5xl font-black mt-2">{{ $students->count() }}</h4>
                    <p class="text-indigo-200 text-xs mt-2 uppercase tracking-widest font-bold">Siswa Aktif</p>
                </div>
            </div>
                        ...
                        <tr>
                            <td colspan="4" class="p-12 text-center">
                                <div class="flex flex-col items-center">
                                    <svg class="w-12 h-12 text-gray-200 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                                    <p class="text-gray-400 italic">Belum ada siswa di kelas ini. Silakan gunakan fitur import.</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
