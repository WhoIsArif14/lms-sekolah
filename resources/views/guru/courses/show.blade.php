    ...
    <div class="py-12" x-data="{
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div x-show="showForm" x-transition class="bg-white p-8 rounded-2xl shadow-md mb-8 border border-gray-200">
                ...
                <form action="{{ route('guru.materials.store', $course) }}" method="POST"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="title" value="{{ old('title') }}"
                                class="w-full rounded-xl border-gray-300 focus:ring-indigo-500 shadow-sm @error('title') border-red-500 @enderror"
                                placeholder="Contoh: Pertemuan 1 - Intro">
                            @error('title')
                    ...
                    <div class="mt-6 p-6 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-300">
                        <div x-show="type === 'file'">
                            <input type="file" name="file_content"
                                class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:bg-indigo-600 file:text-white">
                            @error('file_content')
                                <p class="text-red-500 text-xs mt-1 text-center">{{ $message }}</p>
                            @enderror
                        </div>
                        <div x-show="type === 'video_link'">
                            <input type="url" name="link_content" value="{{ old('link_content') }}"
                    ...
                    <div class="mt-6 flex justify-end">
                        <button type="submit"
                            class="bg-indigo-600 text-white px-10 py-3 rounded-xl font-bold hover:bg-indigo-700 shadow-lg transition">SIMPAN
                            MATERI</button>
                    </div>
                </form>
            </div>
            ...
            <div x-show="showAssignForm" x-transition
                class="bg-white p-8 rounded-2xl shadow-md mb-8 border border-gray-200">
                <h3 class="text-xl font-bold text-gray-800 mb-6 flex items-center">
                ...
                <form action="{{ route('guru.courses.assignments.store', $course) }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    {{-- jika form diakses melalui halaman course, kita tidak perlu field course_id karena sudah ada route param --}}
                    <div class="space-y-6">
                        <div>
                        ...
                        <div class="max-w-xs">
                            <label class="block text-sm font-bold text-gray-700 mb-2">Batas Pengumpulan
                                (Deadline)</label>
                        ...
                        <div class="mb-4">
                            <label class="block text-sm font-bold text-gray-700 mb-2">Link Tambahan (Opsional)</label>
                            <input type="url" name="link" value="{{ old('link') }}"
                                class="w-full rounded-xl border-gray-300" placeholder="https://example.com">
                            @error('link')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="mb-6">
                            <label class="block text-sm font-bold text-gray-700 mb-2">Upload Soal/Modul (PDF/Doc)
                                (Opsional)</label>
                    ...
                    <div class="mt-8 flex justify-end border-t pt-6">
                        <button type="submit"
                            class="bg-yellow-500 text-gray-900 px-10 py-3 rounded-xl font-bold hover:bg-yellow-600 shadow-lg transition uppercase tracking-wider text-sm">
                            POSTING TUGAS
                        </button>
                    </div>
                </form>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="bg-white shadow-sm rounded-2xl p-6 border border-gray-200">
                    <h3 class="font-bold text-lg mb-6 text-gray-800 flex items-center justify-between border-b pb-4">
                        ...
                        <span class="text-xs bg-gray-100 px-2 py-1 rounded text-gray-500">{{ $materials->count() }}
                            Item</span>
                    </h3>
                    <div class="space-y-4">
                        @forelse($materials as $material)
                            <div
                                class="p-4 border border-gray-100 rounded-2xl hover:bg-gray-50 transition shadow-sm flex justify-between items-center">
                                <div class="flex-1 min-w-0">
                                    <p class="font-bold text-gray-700 truncate">{{ $material->title }}</p>
                                    <span
                                        class="text-[10px] px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded-full font-bold uppercase tracking-tighter">
                                        {{ $material->type }}
                                    </span>
                                </div>
                                <div class="flex space-x-2">
                                    <a href="{{ $material->type == 'file' ? asset('storage/' . $material->content) : $material->content }}"
                                        target="_blank"
                                        class="p-2 bg-gray-100 text-gray-600 rounded-lg hover:bg-indigo-600 hover:text-white transition shadow-sm">
                                    ...
                                    <form action="{{ route('guru.materials.destroy', $material->id) }}"
                ...
                <div class="bg-white shadow-sm rounded-2xl p-6 border border-gray-200">
                    <h3 class="font-bold text-lg mb-6 text-gray-800 flex items-center justify-between border-b pb-4">
                        ...
                        <span class="text-xs bg-gray-100 px-2 py-1 rounded text-gray-500">{{ $assignments->count() }}
                            Item</span>
                    </h3>
                    <div class="space-y-4">
                        @forelse($assignments as $assignment)
                            <div
                                ...
                                <form action="{{ route('guru.assignments.destroy', $assignment->id) }}"
                                    method="POST" onsubmit="return confirm('Hapus tugas?')">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                                        class="p-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-600 hover:text-white transition">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor"
